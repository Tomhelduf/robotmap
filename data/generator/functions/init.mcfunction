
scoreboard objectives add generator dummy

scoreboard objectives add path_depth dummy
scoreboard objectives add path_low dummy
scoreboard objectives add path_id dummy
scoreboard objectives add path_parent_id dummy
scoreboard objectives add tile_id dummy

scoreboard players set x_size generator 9
scoreboard players set y_size generator 9

# Progress per tick
scoreboard players set ppt generator 1

scoreboard players set path_limit generator 23